...
static int rockpi_mcu_i2c_read(struct rockpi_mcu_data *ts, u8 reg)
{
	return i2c_smbus_read_byte_data(ts->client, reg);
}

static void rockpi_mcu_i2c_write(struct rockpi_mcu_data *ts,
				      u8 reg, u8 val)
{
	int ret;
...
{
	int ret;
	ret = rockpi_mcu_i2c_read(mcu_data, REG_ID);
	if (ret < 0)
		goto error;
...
int rockpi_mcu_screen_power_up(void)
{
	...
	is_mcu_power_on =0;
	rockpi_mcu_i2c_write(g_mcu_data, REG_POWERON, 0);

	msleep(800);

	rockpi_mcu_i2c_write(g_mcu_data, REG_POWERON, 1);
	/* Wait for nPWRDWN to go low to indicate poweron is done. */
	for (i = 0; i < 100; i++) {
		if (rockpi_mcu_i2c_read(g_mcu_data, REG_PORTB) & 1)
			break;
	}
	rockpi_mcu_i2c_write(g_mcu_data, REG_PORTA, BIT(2));
	is_mcu_power_on =1;
...
int rockpi_mcu_set_bright(int bright)
{
	...
	if (bright==0) {
		rockpi_mcu_i2c_write(g_mcu_data, REG_PWM, 0);
		rockpi_mcu_i2c_write(g_mcu_data, REG_POWERON, 0);
		udelay(1);
		is_mcu_power_on=0;
	} else {
		rockpi_mcu_i2c_write(g_mcu_data, REG_PWM, bright);
	}
...
int rockpi_mcu_is_mcu_power_on(void)
{
	return is_mcu_power_on;
}
EXPORT_SYMBOL_GPL(rockpi_mcu_is_mcu_power_on);


static ssize_t rockpi_mcu_bl_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t rockpi_mcu_bl_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int value;
	...
	if((value < 0) || (value > 255)) {
		...
		rockpi_mcu_set_bright(value);

	return strnlen(buf, count);
}
static DEVICE_ATTR(rockpi_mcu_bl, S_IRUGO | S_IWUSR, rockpi_mcu_bl_show, rockpi_mcu_bl_store);
			...
			const struct i2c_device_id *id)
{
	...
	ret = device_create_file(&client->dev, &dev_attr_rockpi_mcu_bl);
	if (ret != 0) {
		dev_err(&client->dev, "Failed to create rockpi_mcu_bl sysfs files %d\n", ret);
		return ret;
	}
...
static void rockpi_mcu_shutdown(struct i2c_client *client)
{
	is_mcu_power_on = 0;
	rockpi_mcu_i2c_write(g_mcu_data, REG_POWERON, 0);
	return;
...
static int __init rockpi_mcu_init(void)
{
	register_reboot_notifier(&bl_reboot_notifier);
	return i2c_add_driver(&rockpi_mcu_driver);
}

static void __exit rockpi_mcu_exit(void)
{
	unregister_reboot_notifier(&bl_reboot_notifier);
	i2c_del_driver(&rockpi_mcu_driver);
}

subsys_initcall_sync(rockpi_mcu_init);
module_exit(rockpi_mcu_exit);
